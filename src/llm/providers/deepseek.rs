...
/// Wire-level value of the only tool kind `DeepSeek` accepts.
...
#[async_trait]
impl BaseLLMClient for DeepSeek {
...
/// Project the public [`Message`] slice into the wire-shape the
/// `DeepSeek` API expects (OpenAI-compatible, with the
/// `{type:"function", function:{...}}` wrapper around tool calls).
fn wire_messages(messages: &[Message]) -> Vec<WireReqMessage<'_>> {
    messages.iter().map(WireReqMessage::from).collect()
}

/// Project the public [`Tool`] slice into the wire-shape `DeepSeek`
