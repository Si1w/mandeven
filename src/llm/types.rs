...
///
/// Mirrors the wire-level role enumeration the chat-completion APIs
/// expose. The internal [`Message::Compact`] variant has no wire
/// equivalent, so it projects down to [`Self::System`].
...
pub enum Message {
    ...
    /// Synthetic message produced by [`crate::agent::compact`] — a summary
    /// string that replaces a swath of older history, plus a small
    /// metadata bag describing where the boundary came from.
    ///
    /// Mirrors Claude Code's `SystemCompactBoundaryMessage` (per
    /// `agent-examples/claude-code-analysis`'s `messages.ts:4530`)
    /// but expressed as a sum-type variant rather than a TypeScript
    /// `subtype` discriminator. On the wire each provider serializes
    /// this as a plain `system` message; the in-process distinction
    /// lets the UI render it differently and gives session replay a
    /// reliable "this section is a summary" marker.
    ///
    /// Stored verbatim in the session JSONL so reloads see exactly
    /// the same compaction boundary the model saw on the request
    /// that produced it.
    Compact(CompactBoundary),
}

/// Body of a [`Message::Compact`] variant. See that variant's
/// docstring for the role this plays in the compact pipeline.
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct CompactBoundary {
    /// Summary text generated by the compact LLM call.
    pub summary: String,
    /// Whether the compaction was kicked off by `/compact` or by
    /// hitting the auto-trigger threshold.
    pub trigger: CompactTrigger,
    /// Token count estimated for the pre-compact history (the
    /// messages this boundary replaced).
    pub pre_tokens: u32,
    /// Number of messages summarized away.
    pub messages_summarized: usize,
}

/// What kicked off a compaction.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize)]
#[serde(rename_all = "lowercase")]
pub enum CompactTrigger {
    /// User typed `/compact`.
    Manual,
    /// Auto-trigger threshold hit during agent loop.
    Auto,
}
...
impl Message {
    ...
    pub fn role(&self) -> Role {
        match self {
            Self::System { .. } | Self::Compact(_) => Role::System,
            Self::User { .. } => Role::User,
